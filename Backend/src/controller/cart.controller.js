...
export const CartController = {
  ...
  async addToCart(req, res) {
    try {
        ...
        if (!MaKH) return res.status(401).json({ message: "Lỗi Token: Không tìm thấy mã khách hàng" });
        if (!MaSach) return res.status(400).json({ message: "Thiếu mã sách" });

        // Logic thêm giỏ hàng
        let cart = await CartService.getCartByCustomer(MaKH);
        let MaGioHang = cart ? cart.MaGioHang : await CartService.createCart(MaKH);
  ...
  async updateItem(req, res) {
    try {
        ...
        if (cart) {
            if (SoLuong <= 0) await CartDetailService.removeItem(cart.MaGioHang, MaSach);
            else await CartDetailService.updateItem(cart.MaGioHang, MaSach, SoLuong);
        }
  ...
  async get(req, res) {
      try {
          const MaKH = req.user.MaKH;
          const cart = await CartService.getCartByCustomer(MaKH);
          if (!cart) return res.status(200).json({ status: "OK", data: [], total: 0 });
  ...
  async create(req, res) { /* ... giữ nguyên hoặc bỏ qua ... */ }
};