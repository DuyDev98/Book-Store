...
function renderBooks(container, books) {
  ...
  if (!books || !books.length) { 
    container.innerHTML = `<p class="text-center py-5 text-muted">Không tìm thấy sách.</p>`; 
    return; 
  }
  const root = getRootPrefix();
  ...
  books.forEach(b => {
      ...
      html += `<div class="col-6 col-md-3 mb-4"><div class="card h-100 shadow-sm border-0"><div class="p-3 text-center"><a href="${detailUrl}?id=${b.MaSach}"><img src="${img}" class="card-img-top" style="height:200px;object-fit:contain"></a></div><div class="card-body"><h6 class="text-truncate"><a href="${detailUrl}?id=${b.MaSach}" class="text-dark fw-bold text-decoration-none">${b.TenSach}</a></h6><p class="text-danger fw-bold mb-1">${formatCurrency(b.GiaBan)}</p><button class="btn btn-outline-danger w-100 btn-sm" onclick="addToCart(${b.MaSach})">Thêm vào giỏ</button></div></div></div>`;
  });
