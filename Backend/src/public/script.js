...
function overrideUserAlert() {
  ...
  window.alert = function (message) {
    ...
    const isError =
      ...
      msgStr.includes("thất bại") ||
      msgStr.includes("không thể") ||
...
try {
  ...
  if (userStr && userStr !== "undefined")
    CURRENT_USER_INFO = JSON.parse(userStr);
} catch (e) {}
    ...
    if (file.includes("header")) {
      ...
      if (tr) {
        if (CURRENT_USER_INFO) {
          ...
          if (loginBtn)
            loginBtn.onclick = () =>
              (window.location.href = root + "components/login.html");
          if (registerBtn)
            registerBtn.onclick = () =>
              (window.location.href = root + "components/register.html");
        }
      }
      updateCartBadge();
      const sBtn = el.querySelector("#search-btn"),
        sInp = el.querySelector("#search-input");
      if (sBtn && sInp) {
        const doS = () => {
          if (sInp.value.trim())
            window.location.href =
              root +
              `pages/search.html?q=${encodeURIComponent(sInp.value.trim())}`;
        };
        sBtn.onclick = doS;
        sInp.onkeypress = (e) => {
          if (e.key === "Enter") doS();
...
function renderBooks(container, books) {
  ...
  books.forEach((b) => {
    ...
    const badgeHTML = isSale
      ? `<span class="badge bg-danger position-absolute top-0 start-0 m-2">-${b.PhanTramGiamGia}%</span>`
      : "";

    // Nếu có sale: Hiện GiaSale màu đỏ, GiaBan gạch ngang. Nếu không: Hiện GiaBan bình thường.
    const priceHTML = isSale
      ? `<div class="d-flex align-items-center justify-content-between mb-2">
                <span class="text-danger fw-bold">${formatCurrency(
                  b.GiaSale
                )}</span>
                <small class="text-decoration-line-through text-muted" style="font-size:11px">${formatCurrency(
                  b.GiaBan
                )}</small>
            </div>`
      : `<div class="mb-2"><span class="text-danger fw-bold">${formatCurrency(
          b.GiaBan
        )}</span></div>`;
...
async function updateCartBadge() {
  ...
  try {
    ...
    if (!checkAuthError(res)) {
      ...
      badge.style.display =
        d.status === "OK" && d.data.length > 0 ? "block" : "none";
    }
  } catch (e) {}
}
...
async function loadCartPage() {
  ...
  try {
    ...
    d.data.forEach((i) => {
      let img =
        i.AnhBia && i.AnhBia !== "null" ? i.AnhBia : "https://placehold.co/100";
      if (!img.startsWith("http")) img = root + img;
      html += `<tr class="border-bottom"><td class="ps-4 text-start"><div class="d-flex align-items-center"><img src="${img}" style="width:60px;height:80px;object-fit:cover" class="me-3 border"><h6>${
        i.TenSach
      }</h6></div></td><td>${formatCurrency(
        i.GiaBan
      )}</td><td><div class="input-group input-group-sm m-auto" style="width:100px"><button class="btn btn-outline-secondary" onclick="changeQty(${
        i.MaSach
      }, ${
        i.SoLuong - 1
      })">-</button><input class="form-control text-center bg-white" value="${
        i.SoLuong
      }" readonly><button class="btn btn-outline-secondary" onclick="changeQty(${
        i.MaSach
      }, ${
        i.SoLuong + 1
      })">+</button></div></td><td class="text-danger fw-bold">${formatCurrency(
        i.ThanhTien
      )}</td><td><button class="btn text-danger" onclick="removeItem(${
        i.MaSach
      })"><i class="bi bi-trash3-fill"></i></button></td></tr>`;
    });
    tbody.innerHTML = html;
    if (document.getElementById("cart-total-price"))
      document.getElementById("cart-total-price").innerText = formatCurrency(
        d.tongTien || 0
      );
    if (document.getElementById("cart-count-item"))
      document.getElementById("cart-count-item").innerText = d.data.length;
  } catch (e) {}
}

window.changeQty = async function (id, q) {
  if (q < 1) {
    if (confirm("Xóa?")) removeItem(id);
    return;
  }
  await fetch(`${API_BASE_URL}/cart/update`, {
    method: "POST",
    headers: getAuthHeaders(),
    body: JSON.stringify({ MaSach: id, SoLuong: q }),
  });
  loadCartPage();
};
  ...
  try {
    ...
    containers.forEach((container) => {
      ...
      if (slug === "search") {
        ...
        list = k
          ? allBooks.filter((b) => b.TenSach.toLowerCase().includes(k))
          : [];
      }
      // LỌC SÁCH GIẢM GIÁ DỰA TRÊN PHẦN TRĂM > 0
      else if (slug === "flash-sale") {
        list = allBooks
          .filter((b) => b.PhanTramGiamGia > 0)
          .sort((a, b) => b.PhanTramGiamGia - a.PhanTramGiamGia);
      } else if (slug === "hot-sale") {
        list = allBooks.slice(-10).reverse();
      } else if (slug === "recommended") {
        list = [...allBooks].sort(() => 0.5 - Math.random()).slice(0, 10);
...
async function loadSimilarBooks(currentBook) {
  ...
  try {
    ...
    const similarBooks = allBooks.filter(
      (b) =>
        b.MaLoaiSach == currentBook.MaLoaiSach && b.MaSach != currentBook.MaSach
    );
    ...
    if (limitList.length === 0) {
      container.innerHTML =
        '<p class="text-center small text-muted">Chưa có sản phẩm tương tự.</p>';
      return;
    }
      ...
      html += `
            <div class="d-flex mb-3 align-items-center border-bottom pb-2">
                <a href="detail-book.html?id=${b.MaSach}">
                    <img src="${img}" class="rounded border me-2" alt="${
        b.TenSach
      }" style="width: 50px; height: 70px; object-fit: cover;">
                </a>
                <div>
                    <a href="detail-book.html?id=${
                      b.MaSach
                    }" class="text-decoration-none text-dark small fw-bold d-block text-truncate" style="max-width: 130px;" title="${
        b.TenSach
      }">
                        ${b.TenSach}
                    </a>
                    <span class="text-danger fw-bold small">${formatCurrency(
                      currentPrice
                    )}</span>
                </div>
            </div>`;
    ...
    container.innerHTML =
      '<p class="text-center small text-danger">Lỗi tải dữ liệu.</p>';
  }
}
/* ==========================================================================
...
async function loadBookDetail() {
  ...
  try {
    ...
    if (priceEl) {
      ...
      const displayPrice =
        book.PhanTramGiamGia > 0 ? book.GiaSale : book.GiaBan;
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  document
    .querySelectorAll("[data-component-file]")
    .forEach((el) =>
